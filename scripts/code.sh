...
function code-wsl()
{
	...
	# in a wsl shell
	ELECTRON="$ROOT/.build/electron/Systematic.exe"
	if [ -f "$ELECTRON"  ]; then
		local CWD=$(pwd)
		cd $ROOT
		export WSLENV=ELECTRON_RUN_AS_NODE/w:VSCODE_DEV/w:$WSLENV
		local WSL_EXT_ID="ms-vscode-remote.remote-wsl"
		local WSL_EXT_WLOC=$(echo "" | VSCODE_DEV=1 ELECTRON_RUN_AS_NODE=1 "$ROOT/.build/electron/Systematic.exe" "out/cli.js" --locate-extension $WSL_EXT_ID)
		cd $CWD
